...
export default function SoldItemsPage() {
  ...
  const totalEarned = items.reduce((sum, item) => sum + item.price, 0)
      ...
      <main className="main-content">
        <div className="max-w-6xl mx-auto px-4 py-8">
          <div className="content-overlay p-8">
            <h1 className="heading-1 mb-6">💼 MY SOLD ITEMS</h1>

            {/* Summary Statistics */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
              <div className="bg-white/90 backdrop-blur-sm rounded-lg p-6 shadow-sm">
                <h3 className="font-cornerstone text-lg mb-2">ITEMS SOLD</h3>
                <p className="text-3xl font-cornerstone text-blue-600">{itemCount}</p>
              </div>
              <div className="bg-white/90 backdrop-blur-sm rounded-lg p-6 shadow-sm">
                <h3 className="font-cornerstone text-lg mb-2">TOTAL EARNED</h3>
                <p className="text-3xl font-cornerstone text-green-600">${totalEarned.toFixed(2)}</p>
              </div>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
              {items.length === 0 && (
                <p className="description-text col-span-full text-center">No items sold yet.</p>
              )}
              {items.map(item => (
